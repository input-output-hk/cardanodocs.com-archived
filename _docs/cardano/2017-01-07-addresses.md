...
There are currently only two types of addresses in Cardano: `PubKeyAddress` and
...
See more on [hash functions](https://en.wikipedia.org/wiki/Hash_function). See
also sections on [`PubKeyAddress`](#public-key-addresses) and
...
As mentioned in the [Introduction](/#you-own-your-money), the wallets you can
see in the user interface are a convenient representation of the fact that you
own a secret key to spend money in this particular wallet. But how is such
...
address, we create a validator script that can take a so-called redemption
script as a parameter. To redeem funds, we pass the redemption script to the
